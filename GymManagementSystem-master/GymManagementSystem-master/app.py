...
def login():
    if request.method == 'POST':
        ...
        result = cur.execute(
            'SELECT * FROM info WHERE username = %s', [username])
        # print(result)
        if result > 0:
            data = cur.fetchone()
...
class ChangePasswordForm(Form):
    ...
    new_password = PasswordField('Password', [
        ...
        validators.EqualTo(
            'confirm', message='Passwords aren\'t matching pal!, check \'em')
    ])
...
def update_password(username):
    ...
    if request.method == 'POST' and form.validate():
        ...
        cur = mysql.connection.cursor()
        cur.execute(
            "SELECT password FROM info WHERE username = %s", [username])
...
def addTrainor():
    ...
    if request.method == 'POST' and form.validate():
        ...
        cur.execute("INSERT INTO info(name, username, password, street, city, prof, phone) VALUES(%s, %s, %s, %s, %s, %s, %s)",
...
class DeleteRecepForm(Form):
    username = SelectField(
        u'Choose which one you wanted to delete', choices=choices)
...
def deleteTrainor():
    ...
    if request.method == 'POST':
        ...
        username = form.username.data
        q = cur.execute(
            "SELECT username FROM trainors WHERE username != %s", [username])
        b = cur.fetchall()
        new = b[0]['username']
        cur.execute(
            "UPDATE members SET trainor = %s WHERE trainor = %s", (new, username))
...
def addEquip():
    ...
    if request.method == 'POST' and form.validate():
        ...
        if name in equips:
            cur.execute(
                "UPDATE equip SET count = count+%s WHERE name = %s", (count, name))
        else:
            cur.execute(
                "INSERT INTO equip(name, count) VALUES(%s, %s)", (name, count))
        mysql.connection.commit()
        cur.close()
...
def addMember():
    ...
    if request.method == 'POST' and form.validate():
        ...
        cur.execute("INSERT INTO members(username, plan, trainor) VALUES(%s, %s, %s)",
...
class trainorForm(Form):
    ...
    rate = RadioField('Result', choices=[
...
def trainorDash():
    ...
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        p = cur.execute(
            "SELECT date FROM progress WHERE username = %s", [username])
        entered = []
        q = cur.fetchall()
        for i in range(p):
...
def updatePlans():
    ...
    if request.method == 'POST' and form.validate():
        ...
        cur = mysql.connection.cursor()
        cur.execute(
            "SELECT name, exercise FROM plans WHERE name = %s and exercise = %s", (name, exercise))
        result = cur.fetchall()
        if len(result) > 0:
...
def memberDash(username):
    ...
    cur.execute(
        "SELECT exercise, reps, sets FROM plans WHERE name = %s", [plan])
    scheme = cur.fetchall()
    n = cur.execute(
        "SELECT date, daily_result, rate FROM progress WHERE username = %s ORDER BY date DESC", [username])
